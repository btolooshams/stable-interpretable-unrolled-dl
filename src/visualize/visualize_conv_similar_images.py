...
visualize conv similar images for the model x = Dz
...
def main():
    ...
    print("Visualize similar images on conv model x = Dz.")
    ...
    Z_train_proc = Z_train_proc[sorted_indices]
    Z_test_proc = Z_test_proc[sorted_indices]

    Z_train = Z_train[:, sorted_indices]
    Z_test = Z_test[:, sorted_indices]
    ...
    if 1:
        for i in range(20):
            ...
            utils.visualizations.visualize_conv_most_similar_trainig_examples_based_on_code_similarity_and_feature_maps(
                Z_train,
                z_new,
                Z_train_proc,
                z_new_proc,
