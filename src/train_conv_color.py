...
train the conv model x = Dz for color
...
def init_params():
    ...
    parser.add_argument(
        "-i", "--num-ch", type=int, help="number of input channels", default=3,
    )
...
def main():
    ...
    print("Train color model x = Dz for conv denoising.")
    ...
    if params["init_model_path"]:
        ...
        elif params["network"] == "CAElearnbiasstep":
            net = model.CAElearnbiasstep(params)
        elif params["network"] == "CAElearnbiasuntied":
            net = model.CAElearnbiasuntied(params)
        else:
