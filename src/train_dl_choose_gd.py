...
def main():
    ...
    for epoch in tqdm(range(params["num_epochs"]), disable=True):
        ...
        for idx, (x, zstar) in tqdm(enumerate(train_loader), disable=False):
            ...
            # error -----------------------------------------------------#
            with torch.no_grad():
                zT_zstar_err = torch.mean(
                    torch.sqrt(torch.sum((zT - zstar) ** 2, dim=1))
                ).item()
                writer.add_scalar("dl/zT-zstar-err", zT_zstar_err, ctr)
                ...
                if (ctr + 1) % 5 == 0:
                    ...
                    plt.legend()
                    writer.add_figure("dl/code", fig, ctr)
                    plt.close()

                D_Dstar_err_relative = utils.utils.fro_distance_relative(
                    D.clone(), net.W.data.clone()
                )
                writer.add_scalar("dl/D-Dstar-err-relative", D_Dstar_err_relative, ctr)

                writer.add_scalar("dl/train-loss", loss.item(), ctr)
